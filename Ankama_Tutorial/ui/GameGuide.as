package Ankama_Tutorial.ui
{
   ...
   public class GameGuide
   {
      ...
      private var _descriptionWrappers:Array;
      ...
      public function main(params:Object) : void
      {
         ...
         for each(featureId in allFeatureDescriptionId)
         {
            ...
            this._descriptionWrappers.push(featureWrapper);
            if(featureWrapper.parentId == 0)
            {
               this.addEmptyLine(this._descriptionWrappers,featureWrapper.totalHeight + 40);
            }
            else
            {
               this.addEmptyLine(this._descriptionWrappers,featureWrapper.totalHeight);
            }
         }
         ...
         this.gd_descriptions.dataProvider = this._descriptionWrappers;
      ...
      public function updateCategories(data:*, componentsRef:*, selected:Boolean, line:uint) : void
      {
         ...
         {
            case CTR_CAT:
               if(!this._componentList[componentsRef.btn_cat.name])
               {
               ...
               this._componentList[componentsRef.btn_cat.name] = data;
               if(!this._componentList[componentsRef.btn_deployCat.name])
               {
               ...
               this._componentList[componentsRef.btn_deployCat.name] = data;
               ...
               break;
            case CTR_SUB_CAT:
               if(!this._componentList[componentsRef.btn_subCat.name])
               {
               ...
               this._componentList[componentsRef.btn_subCat.name] = data;
               if(!this._componentList[componentsRef.btn_deploySubCat.name])
               {
               ...
               this._componentList[componentsRef.btn_deploySubCat.name] = data;
            ...
            case CTR_SUB_SUB_CAT:
               if(!this._componentList[componentsRef.btn_subSubCat.name])
               {
               ...
               this._componentList[componentsRef.btn_subSubCat.name] = data;
      ...
      private function addSecondaryFeatureDescription(parentId:uint, allIds:Vector.<uint>, parentIndex:uint) : int
      {
         ...
         for(var i:uint = 0; i < featuresIds.length; i++)
         {
            if(allIds.indexOf(featuresIds[i]) == -1)
            {
               ...
               totIndex = totIndex + this.addSecondaryFeatureDescription(featuresIds[i],allIds,parentIndex + 1 + totIndex);
               totIndex++;
            }
         }
         return totIndex;
      ...
      private function addSubCategories(featureDescription:FeatureDescription) : void
      {
         ...
         {
            subCats.push(this.dataApi.getFeatureDescriptionById(subId));
         }
      ...
      {
         for(var i:uint = 0; i < this._descriptionWrappers.length; i++)
         {
            if(this._descriptionWrappers[i] != null && this._descriptionWrappers[i].id == descriptionId)
            {
               return i;
            }
         }
      ...
      private function onSearchTimer(r:TimerEvent = null) : void
      {
         ...
         for each(featureWrapper in this._descriptionWrappers)
         {
            ...
            if(featureWrapper && featureDescriptionWithSearchCriteria.indexOf(featureWrapper.id) != -1)
            {
               ...
               this._totalSearchResult = this._totalSearchResult + featureWrapper.searchResult.length;
      ...
      private function sortByPriority(feature1:Object, feature2:Object) : int
      {
      ...
      private function sortByName(feature1:Object, feature2:Object) : int
      {
