   ...
   public class WorldRpFightTooltipUi
   {
      ...
      public function main(oParam:Object = null) : void
      {
         ...
         for each(fighter in oParam.data.fighters)
         {
            ...
            fightersList = fightersList + (fighter.name + " (");
            if(fighter.id > 0 && fighter.level > ProtocolConstantsEnum.MAX_LEVEL)
            {
               fightersList = fightersList + (this.uiApi.getText("ui.common.short.level") + ProtocolConstantsEnum.MAX_LEVEL + ")\n");
            }
            else
            {
               fightersList = fightersList + (this.uiApi.getText("ui.common.short.level") + fighter.level + ")\n");
            }
         }
