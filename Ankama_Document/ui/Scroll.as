   ...
   public class Scroll extends DocumentBase
   {
      ...
      public function main(params:Object) : void
      {
         ...
         if(this._showBackgroundImage)
         {
            ...
            this._illuX = this._illuX * this.MULTIPLIER;
            this._illuY = this._illuY * this.MULTIPLIER;
            this._contentX = this._contentX * this.MULTIPLIER;
            this._contentY = this._contentY * this.MULTIPLIER;
            this._contentXWithIllu = this._contentXWithIllu * this.MULTIPLIER;
            this.lbl_content.width = this.lbl_content.width * this.MULTIPLIER;
            this.lbl_content.height = this.lbl_content.height * this.MULTIPLIER;
            this.mask_container.height = this.lbl_content.height;
            this._lbl_title = this.lbl_title2;
            this.tx_cartouche.scale = this.MULTIPLIER;
            this.btn_close.x = this.btn_close.x * this.MULTIPLIER;
            this.btn_close.y = this.btn_close.y * this.MULTIPLIER;
            this.mainCtr.x = 107;
            this.mainCtr.y = 80;
         }
         else
      ...
      private function _initScroll() : void
      {
         ...
         for(var i:int = 0; i < this._numLines; i++)
         {
            ...
            h = h + this.lbl_content.textfield.getLineMetrics(i).height;
      ...
      private function applyProperties() : void
      {
         ...
         if(this._properties)
         {
            ...
            if(this._properties.hasOwnProperty("titleOffsetY"))
            {
               ...
               this._lbl_title.y = this._lbl_title.y + offsetY;
               this.tx_deco.y = this.tx_deco.y + offsetY;
            }
      ...
      private function displayPage(pPage:int) : void
      {
         ...
         for(var i:uint = 0; i < this._numLines; i++)
         {
            ...
            {
               pageOffset = pageOffset + textHeight;
            }
            else if(maskHeight + textHeight < this.mask_container.height)
            {
               maskHeight = maskHeight + textHeight;
            }
            else
            {
               break;
      ...
      public function onTextureReady(pTexture:Object) : void
      {
         var r:Object = null;
         var p:Point = null;
