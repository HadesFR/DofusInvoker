package Ankama_CharacterSheet.ui
{
   ...
   public class CharacterBuildsUi
   {
      ...
      private var _textureMinusUri:Object;
      
      private var _texturePlusUri:Object;
      ...
      public function updateCaracLine(data:*, componentsRef:*, selected:Boolean) : void
      {
         ...
         if(data)
         {
            ...
            {
               if((statpoints[i + 1] && statpoints[i + 1][0] > data.base && statpoints[i][0] <= data.base || !statpoints[i + 1]) && this._characterCharacteristics.statsPoints >= statpoints[i][1] && isRoleplayOrPrefight)
               {
                  componentsRef.btn_plus.visible = true;
               }
               if(data.additionnal < ProtocolConstantsEnum.MAX_ADDITIONNAL_PER_CARAC && (statpoints[i + 1] && statpoints[i + 1][0] > data.additionnal && statpoints[i][0] <= data.additionnal || !statpoints[i + 1]) && this._characterCharacteristics.additionnalPoints >= statpoints[i][1] && isRoleplayOrPrefight)
               {
                  componentsRef.btn_plus.visible = true;
               }
            }
      ...
      public function updateCategory(data:*, componentsRef:*, selected:Boolean, line:uint) : void
      {
         ...
         switch(this.getCatLineType(data,line))
         {
            ...
            case CTR_CAT_TYPE_ITEM:
               ...
               {
                  currentBase = this._characterCharacteristics[data.keyword + "Current"] - (data.boost + data.stuff);
                  if(currentBase < 0)
                  {
      ...
      private function updateStats() : void
      {
         ...
         if(this._currentUiState == READ_ONLY_STATE)
         {
            ...
            for each(data in this._statsData)
            {
               ...
               {
                  carac = this._characterCharacteristics[data.keyword];
                  caracBaseValue = carac is CharacterBaseCharacteristic?int(carac.base):int(int(carac));
         ...
         for each(data in this._statsData)
         {
            ...
            {
               if(this._characterCharacteristics.hasOwnProperty(data.keyword))
               {
                  carac = this._characterCharacteristics[data.keyword];
                  ...
                  if(carac is CharacterBaseCharacteristic)
                  {
                     if(statDataInBuild)
                     {
                        if(basePlayerCaracForceUse)
                        {
                           data.base = carac.base;
                        }
                        else
                        {
                     ...
                     {
                        data.base = carac.base;
                        data.additionnal = carac.additionnal;
                        data.stuff = carac.objectsAndMountBonus;
                     }
                     data.boost = carac.contextModif;
                  }
         ...
         {
            maxLifePoints = this._characterCharacteristics.maxLifePoints;
         }
         else
         {
            ...
            if(basePlayerCaracForceUse)
            {
               maxLifePoints = maxLifePoints + (this._characterCharacteristics.maxLifePoints - this._characterCharacteristics.vitality.base - this._characterCharacteristics.vitality.alignGiftBonus - this._characterCharacteristics.vitality.contextModif - this._characterCharacteristics.vitality.additionnal - this._characterCharacteristics.vitality.objectsAndMountBonus);
            }
            else
            {
               maxLifePoints = maxLifePoints + this._currentBuild.statsByKeyword["lifePoints"].base;
            }
         }
         ...
         {
            statsPoints = this._characterCharacteristics.statsPoints;
         }
         else
         {
            statsPoints = this._currentBuild.statsByKeyword["statsPoints"].base;
         }
         var points:int = !!this._someStatsMayBeScrolled?int(statsPoints + this._characterCharacteristics.additionnalPoints):int(statsPoints);
      ...
      private function isCurrentBuildDifferentFromCharacter() : Boolean
      {
         ...
         var characterEquipment:Object = this.storageApi.getViewContent("equipment");
         ...
         {
            carac = this._characterCharacteristics[upgradableStat.keyword];
            ...
            if(buildCaracteristicsByKeyword[upgradableStat.keyword].base != carac.base)
            {
               this.log(2,"    La valeur de base de la caracteristique " + upgradableStat.keyword + " est " + buildCaracteristicsByKeyword[upgradableStat.keyword].base + " dans le build, " + carac.base + " dans l\'inventaire");
               return true;
            }
         }
      ...
      {
         var temporisSpellsUiContainer:UiRootContainer = null;
         var isRoleplayOrPrefight:Boolean = false;
         var id:int = 0;
         ...
         var superType:int = 0;
         var inventoryItems:Object = null;
         ...
         switch(target)
         {
            ...
            case this.lbl_manageSpellSets:
               temporisSpellsUiContainer = this.uiApi.getUi(UIEnum.TEMPORIS_SPELLS_UI);
               if(!temporisSpellsUiContainer)
               {
                  this.sysApi.sendAction(new OpenTemporisSpellsUiAction([true]));
               }
               else if(!this.uiApi.getUi(UIEnum.TEMPORIS_SPELL_SETS_UI))
               {
                  temporisSpellsUiContainer.uiClass.getSpellSets();
               }
               break;
            default:
               if(this._currentUiState == MODIFY_STATE)
               {
                  ...
                  else if(target.name.indexOf("slot_") == 0)
                  {
                     ...
                     if(target.data && target.data is ItemWrapper)
                     {
                        this._equippedItemClicked = (target.data as ItemWrapper).objectUID;
                     }
                     i = 0;
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         if(target is Slot && target.data)
         {
            ...
            {
               this.uiApi.showTooltip(target.data,target,false,"standard",LocationEnum.POINT_BOTTOMRIGHT,LocationEnum.POINT_TOPLEFT,0,"itemName",null,{
                  "noBg":false,
            ...
            this.uiApi.showTooltip(target.data,target,false,"standard",globalPosition.x > 500?uint(LocationEnum.POINT_TOPRIGHT):uint(LocationEnum.POINT_TOPLEFT),globalPosition.x > 500?uint(LocationEnum.POINT_TOPLEFT):uint(LocationEnum.POINT_TOPRIGHT),20,null,null,settings);
            return;
         }
         ...
         switch(target)
         {
            case this.ctr_pointsToDivide:
               ...
               {
                  text = this.uiApi.getText("ui.common.base") + " (" + this.uiApi.getText("ui.charaSheet.natural") + " + " + this.uiApi.getText("ui.charaSheet.additionnal") + ")" + this.uiApi.getText("ui.common.colon") + this._characterCharacteristics.statsPoints + "+" + this._characterCharacteristics.additionnalPoints;
               }
               break;
            case this.btn_reset:
               ...
               else if(target.name.indexOf("lbl_value") != -1)
               {
                  if(this.ctr_advanced.visible)
                  {
                     ...
                     params = {
                        ...
                        "additionalInfo":this.uiApi.getText("ui.common.total") + this.uiApi.getText("ui.common.colon") + target.text,
                        "maxWidth":100
                     };
      ...
      {
         var position:uint = 0;
         if(target is Slot && !(target as Slot).softDisabled && target.data && !this._delayDoubleClickTimer.running)
         {
            this.uiApi.unloadUi("itemsList");
            ...
            {
               this.sysApi.sendAction(new ObjectSetPositionAction([target.data.objectUID,63,1]));
            }
         }
      }
