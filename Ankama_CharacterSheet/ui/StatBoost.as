package Ankama_CharacterSheet.ui
{
   ...
   public class StatBoost
   {
      ...
      public function displayUI(statKeyword:String, statId:int, someStatsCanBeScrolled:Boolean, uiIsIndependant:Boolean = false) : void
      {
         ...
         var playerSpells:Object = this.playerApi.getSpellInventory();
         ...
         this.displayFloors();
         var noAdditionalPoints:Boolean = this._characterCharacteristics.additionnalPoints == 0 || !this._someStatsCanBeScrolled;
      ...
      {
         var characterSheet:* = this.uiApi.getUi("characterSheetUi");
      ...
      private function onEnterFrame(e:Event) : void
      {
         ...
         var characterSheetUiRect:Object = this._characterSheetUi.getStageRect();
      ...
      private function resizeVertically() : void
      {
         ...
         var footerY:int = 0;
         var characterSheetUiRect:Object = null;
      ...
      private function displayPoints() : void
      {
         ...
         var bonus:int = this._characterCharacteristics[this._stat].additionnal + this._characterCharacteristics[this._stat].objectsAndMountBonus;
         this.lbl_statBonus.text = String(bonus);
         this.lbl_statTotal.text = String(this._characterCharacteristics[this._stat].base + this._statPointsAdded + bonus);
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         else if(target.customUnicName.indexOf("lbl_variable") != -1 && this._statVariableAssoc[target])
         {
            ...
            statTotal = this._characterCharacteristics[this._stat].base + this._characterCharacteristics[this._stat].additionnal + this._characterCharacteristics[this._stat].objectsAndMountBonus;
            switch(this._statId)
            {
               case BoostableCharacteristicEnum.BOOSTABLE_CHARAC_WISDOM:
                  statAmount = 10;
            ...
            if(bonusAmount > 0)
            {
               text = this.uiApi.getText("ui.stats.statIncreases.tooltip",this.uiApi.processText(this.uiApi.getText(this._statPointsName,statAmount),"",statAmount <= 1,statAmount == 0),bonusAmount,target.text,this.uiApi.processText(this.uiApi.getText(this._statPointsName,statTotal),"",statTotal <= 1,statAmount == 0));
            }
         }
         if(!text)
         {
