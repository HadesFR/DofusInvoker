package Ankama_Social.ui
{
   ...
   public class Friends
   {
      ...
      public function main(params:Array) : void
      {
         ...
         this.lbl_accountName.text = this.sysApi.getPlayerManager().nickname;
         ...
         var cbxField_1:String = this._chatService.getMyGameActivity(GROUP_KOLI);
         var cbxField_2:String = this._chatService.getMyGameActivity(GROUP_DUNGEON);
         var cbxField_3:String = this._chatService.getMyGameActivity(GROUP_PVM);
      ...
      public function updateCategory(data:*, componentsRef:*, selected:Boolean, line:uint) : void
      {
         ...
         switch(this.getCategoryLineType(data,line))
         {
            ...
            case CTR_ITEM_BLOCKED:
               componentsRef.lbl_nameBloqued.text = data.name;
            ...
            case CTR_FRIEND:
               ...
               componentsRef.btn_addContact.softDisabled = !this._chatService.authenticated;
               ...
               componentsRef.lbl_name.text = "{account," + friendLauncher.user.name + "," + friendLauncher.user.userId + "," + SocialCharacterCategoryEnum.CATEGORY_FRIEND_ANKAMA + "::" + friendLauncher.user.name + "}";
            ...
            case CTR_CONTACT:
               ...
               componentsRef.btn_addAnkamaFriend.softDisabled = !this._chatService.authenticated;
               this.addHookOnGridElements(componentsRef.btn_addAnkamaFriend,data);
               componentsRef.lbl_name.text = !!character.online?"{player," + character.playerName + "," + character.playerId + "," + null + "," + 0 + "," + 0 + "," + character.accountId + "::" + character.name + " (" + character.playerName + ")" + "}":"{account," + character.name + "," + character.accountId + "," + SocialCharacterCategoryEnum.CATEGORY_CONTACT + "::" + character.name + "}";
               if(data.statusId)
               {
      ...
      public function updateWaitLine(data:*, componentsRef:*, selected:Boolean) : void
      {
         if(data)
         {
            if(data.inviter.name == this.sysApi.getPlayerManager().nickname)
            {
               componentsRef.lbl_invitationPlayerName.text = data.recipient.name;
            ...
            {
               componentsRef.lbl_invitationPlayerName.text = data.inviter.name;
         ...
         if(tabToRefresh == this._nCurrentTab)
         {
            ...
            switch(this._nCurrentTab)
            {
               case 0:
                  ...
                  if(!this._chatService.authenticated)
                  {
                     this._dataMatrix = [];
      ...
      {
         var tab:SortType = this._tabsSortType[this.gd_friends.selectedItem.e_category] as Friends;
      ...
      private function sortGrid(category:String = null) : void
      {
         ...
         var index:int = 0;
         this._currentSortElem = this._tabsSortType[category] as Friends;
         if(this._currentSortElem != null)
         {
            startIndex = 0;
            count = 0;
            i = 0;
            while(i < this._dataMatrix.length && !(this._dataMatrix[i] is Friends && this._dataMatrix[i].e_category == category))
            {
               startIndex++;
               i++;
            }
            startIndex++;
            i = startIndex;
            while(i < this._dataMatrix.length && !(this._dataMatrix[i] is Friends))
            {
               count++;
               i++;
            }
      ...
      public function onRelease(target:Object) : void
      {
         ...
         switch(target)
         {
            ...
            default:
               ...
               {
                  this.showAddFriendUI(SocialCharacterCategoryEnum.CATEGORY_FRIEND_ANKAMA,data.name);
               }
               else if(target.name.indexOf("btn_addContact") != -1)
               {
                  this.sysApi.sendAction(new AddFriendAction([data.user.name]));
               }
               break;
         }
      }
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         else if(target.name.indexOf("tx_league") != -1)
         {
            ...
            if(league)
            {
               ...
               {
                  text = text + (" " + this._componentData[target.name].ladderPosition);
               }
            }
            else
            {
