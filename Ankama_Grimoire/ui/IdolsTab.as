package Ankama_Grimoire.ui
{
   ...
   public class IdolsTab
   {
      ...
      private static function getCharacteristicWithBonuses(characteristics:CharacterBaseCharacteristic) : int
      {
         return characteristics.base + characteristics.additionnal + characteristics.objectsAndMountBonus + characteristics.alignGiftBonus + characteristics.contextModif;
      }
      ...
      public function main(oParam:Object = null) : void
      {
         ...
         this._idolsSelections = new Vector.<IdolsTab>(0);
         ...
         this._soloIdols = [];
         var playerIdols:Object = this.inventoryApi.getStorageObjectsByType(DataEnum.ITEM_TYPE_IDOLS);
      ...
      public function updateIdolLine(idolData:Object, components:*, selected:Boolean) : void
      {
         ...
         if(idolData)
         {
            ...
            {
               components.tx_incompatibleMonsters.visible = true;
      ...
      private function getIdolData(pIdol:Object, pOwnersIds:Object, pActive:Boolean = false, pReadOnly:Boolean = false) : Object
      {
         ...
         var item:Object = this.dataApi.getItemWrapper(pIdol.itemId);
         return {
            "idol":pIdol,
            "idolId":pIdol.id,
            "item":item,
      ...
      private function updateTotals() : void
      {
         ...
         this.tx_allIncompatibleMonsters.visible = !!incompatibleMonstersNames?true:false;
      }
      ...
      {
         var wisdom:int = getCharacteristicWithBonuses(this.playerApi.characteristics().wisdom);
      ...
      {
         var pp:int = getCharacteristicWithBonuses(this.playerApi.characteristics().prospecting);
         var level:int = this.playerApi.getPlayedCharacterInfo().level;
         if(level > ProtocolConstantsEnum.MAX_LEVEL)
         {
            level = ProtocolConstantsEnum.MAX_LEVEL;
      ...
      private function addSynergy(pSynergiesList:Dictionary, pSourceIdolId:uint, pOtherIdolId:uint, pOtherIdolCoeff:Number) : void
      {
         ...
         {
            pSynergiesList[pSourceIdolId] = new Vector.<IdolsTab>(0);
         }
      ...
      private function getSynergiesText(pSynergiesList:Dictionary, pPositiveSynergies:Boolean) : String
      {
         ...
         for(idolId in pSynergiesList)
         {
            ...
            {
               text = text + ("<li><b>" + idol.item.name + "</b>" + this.uiApi.getText("ui.common.colon") + synergiesText + "</li>");
      ...
      private function getSynergyText(pIdolId:int, pSynergies:Vector.<IdolSynergy>, pDisplayedIdols:Vector.<int> = null, pSelectedIdolsOnly:Boolean = false) : String
      {
         ...
         for each(synergy in pSynergies)
         {
            if((!pSelectedIdolsOnly || this._activeIdolsIds.indexOf(synergy.idolId) != -1) && (!pDisplayedIdols || pDisplayedIdols.indexOf(synergy.idolId) == -1))
            {
               ...
               synergiesText = synergiesText + ((!!firstSynergyIdol?"":", ") + synergyIdol.item.name + " (x" + "<font color=\'" + (synergy.synergyCoeff > 1?this._synergyBonusColor:this._synergyMalusColor) + "\'>" + synergy.synergyCoeff.toFixed(2) + "</font>" + ")");
      ...
      {
         var allIdols:Object = null;
         var i:int = 0;
         var numIdols:uint = 0;
         var newDp:Array = null;
         var idol:Object = null;
      ...
      private function savePreset(pPreset:Object) : void
      {
         ...
         for(i = 0; i < pPreset.idolsIds.length; i++)
         {
            ...
            presetScore = presetScore + Math.floor(idol.score * coeff);
         ...
         for each(idolData in idolsData)
         {
            ...
            if(this.btn_showSynergyScore.selected)
            {
               if(this._activeIdolsIds.indexOf(idolData.idolId) != -1)
               {
                  ...
                  {
                     currentScore = currentScore + selectedIdolsScores[idolId];
      ...
      private function getIndexFromComponent(pTarget:Object) : int
      {
      ...
      public function onSelectItem(target:Object, selectMethod:uint, isNewSelection:Boolean) : void
      {
         ...
         switch(target)
         {
            case this.gd_activeIdols:
               ...
               {
                  this.selectIdol(this.dataApi.getIdolByItemId(target.selectedItem.objectGID).id,false,this.gd_activeIdols);
               }
               break;
            case this.gd_idols:
               if(!this._isInFight && selectMethod == SelectMethodEnum.CLICK)
               {
                  idolData = target.selectedItem;
            ...
            case this.cb_filter:
               ...
               this._currentFilter = target.selectedItem.id;
               this.updateList();
               break;
            case this.gd_icons:
               this._currentIcon = target.selectedItem.id;
         }
      }
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         {
            this.showIdolTooltip(target.data,target);
         }
         else
         {
      ...
      {
         this.sysApi.sendAction(new PresetUseRequestAction([target.data.id]));
      }
