package Ankama_Grimoire.ui
{
   ...
   public class SpellList
   {
      ...
      public function updateSpellButtonsLine(data:*, componentsRef:*, selected:Boolean) : void
      {
         ...
         if(data)
         {
            ...
            if(!spellCache)
            {
               ...
               if(this.sysApi.getPlayerManager().hasRights)
               {
                  spellCache.lbl_spellName_text = spellCache.lbl_spellName_text + (" (" + data.id + ")");
               }
      ...
      public function onSelectItem(target:Object, selectMethod:uint, isNewSelection:Boolean) : void
      {
         ...
         {
            this._showSpellTypeId = this.cb_spellType.dataProvider[target.selectedIndex].value;
            switch(selectMethod)
            {
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         if(target.name.indexOf("btn_obtention") != -1)
         {
            ...
            for(i = 0; i < 4; i++)
            {
               btnSpellName = btnSpellName + (btnSpellNameArray[i] + "m");
            }
