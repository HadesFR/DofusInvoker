      ...
      private function createMecanic() : void
      {
         ...
         for each(node in formatedDescription.children())
         {
            ...
            if(nodeName == TITLE || nodeName == SUBTITLE)
            {
               if(desc != "")
               {
                  groupPos = groupPos + (!(oldNodeName == TITLE || oldNodeName == SUBTITLE)?marginY:0);
                  groupPos = groupPos + this.addDesc(groupPos,desc);
                  desc = "";
               }
               iconId = 0;
               ...
               groupPos = groupPos + (oldNodeName == "base"?0:iconId > 0 || nodeName == TITLE?marginY:0);
               groupPos = groupPos + this.tx_mecanicIcon.height;
            }
            else
            {
               if(desc != "")
               ...
               desc = desc + (!!node.name()?node.toXMLString():node.toString());
            }
            oldNodeName = nodeName;
         }
         if(desc != "")
         {
            groupPos = groupPos + marginY;
            groupPos = groupPos + this.addDesc(groupPos,desc);
            groupPos = groupPos + marginY;
