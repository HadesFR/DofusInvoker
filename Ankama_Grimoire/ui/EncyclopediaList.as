package Ankama_Grimoire.ui
{
   ...
   public class EncyclopediaList
   {
      ...
      private static const ANIMAL_TYPES:Array = [DataEnum.ITEM_SUPERTYPE_PET,DataEnum.ITEM_SUPERTYPE_CERTIFICATE,DataEnum.ITEM_SUPERTYPE_PET_GHOST];
      ...
      public function main(params:Object) : void
      {
         ...
         if(!this._levelTimer)
         {
            this._levelTimer = new Timer(200,1);
            ...
            case 1:
               for(i = 0; i < typesAvailable.length; i++)
               {
                  if(!(tabIndex == 1 && typesAvailable[i].categoryId != ItemCategoryEnum.EQUIPMENT_CATEGORY || !typesAvailable[i].isInEncyclopedia || typesAvailable[i].superTypeId == DataEnum.ITEM_SUPERTYPE_LIVING_ITEMS || typesAvailable[i].superTypeId == DataEnum.ITEM_SUPERTYPE_COSTUME || typesAvailable[i].superTypeId == DataEnum.ITEM_SUPERTYPE_MOUNT_EQUIPMENT))
                  {
                     ...
                     if(ANIMAL_TYPES.indexOf(typesAvailable[i].superTypeId) != -1)
                     {
                        this.fillTypesArray(supertype,ANIMAL_TYPES[0]);
                     }
                     else
                     {
      ...
      private function parseItem(itemId:uint) : Object
      {
         ...
         if(!tmpItem.hasOwnProperty("etheral") || !tmpItem.etheral)
         {
            ...
            if(tmpItem.recipeSlots != 0 && !tmpItem.secretRecipe)
            {
               item.obtaining.push({
                  "id":1,
                  "item":item.itemWrapper,
            ...
            if(this.sysApi.getCurrentServer().gameTypeId == GameServerTypeEnum.SERVER_TYPE_TEMPORIS && (tmpItem.dropMonsterIds.length > 0 || tmpItem.dropTemporisMonsterIds && tmpItem.dropTemporisMonsterIds.length > 0))
            {
               item.obtaining.push({
                  "id":2,
                  "item":item.itemWrapper,
      ...
      private function parseMount(item:Object) : Object
      {
         var currentItem:Object = new Object();
         var currentMount:Object = this.dataApi.queryEquals(Mount,"certificateId",item.id);
      ...
      {
         var typeId:uint = 0;
         var ids:Object = this.dataApi.queryEquals(ItemType,"superTypeId",superTypeId);
      ...
      public function onSelectItem(target:Object, selectMethod:uint, isNewSelection:Boolean) : void
      {
         ...
         {
            if(selectMethod != GridItemSelectMethodEnum.AUTO && target.selectedItem && target.selectedItem.isCat)
            {
               this._lastFilterCategorySelected = target.selectedItem;
               this.itemFilterApi.displayCategories(target.selectedItem);
            }
         }
      }
         ...
         switch(target)
         {
            ...
            default:
               ...
               else if(target.name.indexOf("btn_itemType") != -1)
               {
                  ...
                  this.btn_resetFilters.disabled = this.itemFilterApi.currentSubIds().length == 0 && this.itemFilterApi.currentFilteredIds().length == 0 && this.itemFilterApi.currentCharacIds().length == 0 && this.itemFilterApi.currentConsumableEffectIds().length == 0 && this.itemFilterApi.currentWeaponEffectIds().length == 0 && this.itemFilterApi.obtainingMethod().length == 0 && this.itemFilterApi.currentSkinTargetIds().length == 0 && !this._searchCriteria;
                  this.lbl_resetFilters.cssClass = !!this.btn_resetFilters.disabled?"lightgreycenter":"greencenter";
                  target.selected = this.itemFilterApi.currentFilteredIds().indexOf(typeData) != -1;
               }
               else if(target.name.indexOf("tx_obtaining") != -1)
               {
                  ...
                  else if(obtainigData.id == ITEM_DROPABLE_CAT_ID)
                  {
                     ...
                     if(this.sysApi.getCurrentServer().gameTypeId == GameServerTypeEnum.SERVER_TYPE_TEMPORIS && obtainigData.item.dropTemporisMonsterIds)
                     {
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         else if(target.name.indexOf("tx_obtaining") != -1)
         {
            ...
            target.uri = this.uiApi.createUri(this.uiApi.me().getConstant("texture") + btnData.iconOverPath);
            switch(btnData.id)
            {
      ...
      public function onRollOut(target:Object) : void
      {
         ...
         {
            btnData = this._componentList[target.name];
            target.uri = this.uiApi.createUri(this.uiApi.me().getConstant("texture") + btnData.iconGreyPath);
         }
