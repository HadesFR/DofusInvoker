package Ankama_GameUiCore.ui
{
   ...
   public class BannerMenu extends ContextAwareUi
   {
      ...
      override public function main(args:Array) : void
      {
         ...
         this._maxBtnIds = !!this._isTemporisSpellsUi?uint(20):uint(19);
         ...
         this.onAddBannerButton(this.ID_BTN_GUIDEBOOK,"btn_progression",this.onGuidebookAction,this.uiApi.getText("ui.guidebook.title"),"openGuidebook");
         ...
         {
            this.onAddBannerButton(this.ID_BTN_KOLIZEUM,"btn_conquest",this.onKolizeumAction,this.uiApi.getText("ui.common.koliseum"),"openPvpArena",this.uiApi.getText("ui.banner.lockBtn.lvl",ProtocolConstantsEnum.CHAR_MIN_LEVEL_ARENA));
         }
      ...
      private function onSpellsAction() : void
      {
         if(this.shortcutTimerReady() && this.getBtnById(this.ID_BTN_SPELL).active)
         {
            if(this._isTemporisSpellsUi)
            {
               ...
               {
                  sysApi.sendAction(new OpenTemporisSpellsUiAction([]));
               }
               else
               {
                  sysApi.sendAction(new OpenBookAction(["temporisSpellsUi"]));
               }
            }
            else
            {
      ...
      private function onKolizeumAction() : void
      {
         ...
         if(this.shortcutTimerReady())
         {
            if(this.getBtnById(this.ID_BTN_KOLIZEUM).active)
            {
      ...
      private function onGuidebookAction() : void
      {
         ...
         if(this.shortcutTimerReady() && this.getBtnById(this.ID_BTN_GUIDEBOOK).active)
         {
            ...
            {
               tab = sysApi.getSetData("lastGuidebookTab",EnumTab.GUIDEBOOK_GAME_GUIDE,DataStoreEnum.BIND_ACCOUNT);
               sysApi.sendAction(new OpenGuidebookAction([tab]));
            }
            else
            {
         ...
         if(uiName && uiName.indexOf("levelUp") == 0 && this.playerApi && this.playerApi.getPlayedCharacterInfo())
         {
            ...
            if(playerInfo.level >= ProtocolConstantsEnum.CHAR_MIN_LEVEL_ARENA && btn && this.checkConquest())
            {
               if(btn.highlight)
               {
                  ...
                  {
                     this.toggleAllButtonsVisibility();
                     this.highlightApi.highlightUi(UIEnum.BANNER_MENU,"gd_additionalBtns|id|" + this.ID_BTN_KOLIZEUM,3,0,5,true);
                  }
                  else
                  {
                     this.highlightApi.highlightUi(UIEnum.BANNER_MENU,"gd_btnUis|id|" + this.ID_BTN_KOLIZEUM,0,0,5,true);
                  }
               }
               btn.highlight = false;
            }
