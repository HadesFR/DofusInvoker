package Ankama_Job.ui
{
   ...
   public class SmithMagic
   {
         ...
         switch(target)
         {
            ...
            default:
               if(target.name != "slot_rune" && target.name.indexOf("slot_rune") == 0 && this.chk_advancedMode.selected && !this.isAdvancedInProgress && !this.uiApi.keyIsDown(Keyboard.CONTROL))
               {
                  ...
                  this._multiCraft = false;
                  this._runeUsedForLastMage = target.data;
                  this.isAdvancedInProgress = true;
                  this.setMergeButtonDisabled(true);
                  this.sysApi.dispatchHook(HookList.DoubleClickItemInventory,target.data,1);
               }
      ...
      public function onMouseCtrlDoubleClick(target:Object) : void
      {
         ...
         if(target.data && (this.uiApi.keyIsDown(Keyboard.CONTROL) || this.uiApi.keyIsDown(15)))
         {
            ...
            else
            {
               ...
               if(iw)
               {
                  this.sysApi.dispatchHook(HookList.DoubleClickItemInventory,target.data,iw.quantity - this.getAlreadyInSlot(iw));
               }
               else
               {
                  this.sysApi.dispatchHook(HookList.DoubleClickItemInventory,target.data,target.data.quantity);
               }
            }
         }
      }
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         if(target.hasOwnProperty("data") && target.data)
         {
            this.uiApi.showTooltip(target.data,target,false,"standard",LocationEnum.POINT_TOPLEFT,LocationEnum.POINT_TOPLEFT,0,"itemName",null,{
               "showEffects":true,
               "header":true
            },"ItemInfo");
            return;
