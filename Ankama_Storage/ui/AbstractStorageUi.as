package Ankama_Storage.ui
{
   ...
   public class AbstractStorageUi
   {
      ...
      {
         this.saveTabState();
         if(!this.questVisible && category == ItemCategoryEnum.QUEST_CATEGORY)
         {
      ...
      public function onRollOver(target:Object) : void
      {
         ...
         switch(target)
         {
            ...
            case this.tx_limitIconHelp:
               ...
               {
                  text = text + (this.uiApi.getText("ui.common.colon") + this.lbl_kamas.text);
               }
               break;
            case this.btn_checkCraft:
      ...
      public function onItemRightClick(target:Object, item:Object) : void
      {
         ...
         this.fillContextMenu(contextMenu.content,data,disabled);
         this.modContextMenu.createContextMenu(contextMenu);
      }
      ...
      public function displayAssociatedRunes(itemW:ItemWrapper, onClickOnItem:Boolean) : void
      {
         ...
         if(!itemW)
         {
            this.cb_category.value = this.cb_category.dataProvider[0];
            this.onRelease(this.btn_closeSearch);
            return;
         }
         if(onClickOnItem && this.cb_category.selectedItem.filterType != DataEnum.ITEM_TYPE_SMITHMAGIC_RUNE)
         {
            return;
         }
         this._lastSearchCriteria = null;
         this.categoryFilter = this.getCategoryFromButton(this.btnRessources);
         if(!onClickOnItem && this.cb_category.selectedItem.filterType != DataEnum.ITEM_TYPE_SMITHMAGIC_RUNE)
         {
            for each(category in this.cb_category.dataProvider)
            {
               if(category.filterType == DataEnum.ITEM_TYPE_SMITHMAGIC_RUNE)
               {
                  this.cb_category.value = category;
                  break;
               }
            }
         }
         var inventory:* = this._inventoryItems;
         var l:int = inventory.length;
         var filteredInventory:Array = [];
         this.itemsDisplayed = new Vector.<uint>();
         var itemEffects:Vector.<EffectInstance> = itemW.effects.concat(itemW.possibleEffects);
         for(var i:int = 0; i < l; i++)
         {
            item = inventory[i];
            if(item.typeId == DataEnum.ITEM_TYPE_SMITHMAGIC_RUNE)
            {
               runeEffects = item.effects.concat(item.possibleEffects);
               for each(itemEffect in itemEffects)
               {
                  for each(runeEffect in runeEffects)
                  {
                     if(this.dataApi.getEffect(itemEffect.effectId).characteristic == this.dataApi.getEffect(runeEffect.effectId).characteristic)
                     {
                        if(this.itemsDisplayed.indexOf(item.objectUID) == -1)
                        {
                           filteredInventory.push(item);
                           this.itemsDisplayed.push(item.objectUID);
                        }
         ...
         this.inp_search.text = this.uiApi.getText("ui.item.runes") + itemW.name;
         this.btn_closeSearch.visible = true;
      ...
      private function replaceCategoryButtons() : void
      {
         ...
         buttons.push(this.btnAll);
         buttons.push(this.btnEquipable);
         buttons.push(this.btnConsumables);
         buttons.push(this.btnRessources);
         ...
         for each(btn in buttons)
         {
            ...
            xForCurrentButtonWithoutOffset = xForCurrentButtonWithoutOffset + (btn.width + 1);
         }
      }
