   ...
   public class FightTexts
   {
      ...
      private static function getSpellName(casterId:Number, targetCellId:int, sourceCellId:int, spellId:uint, spellLevelId:uint, forceDetailedLog:Boolean) : String
      {
         ...
         if(spell && forceDetailedLog)
         {
            ...
            spellName = spellName + (" (" + spellLevel.spellId + ") rang " + spellLevel.grade);
         }
      ...
      private static function getFighterName(id:Number) : String
      {
            ...
            for each(team in teams)
            {
               ...
               switch(team)
               {
                  case "allies":
                     ...
                     {
                        fighterName = fighterName + Api.uiApi.getText("ui.common.attackers");
                     }
                     else
                     {
                        fighterName = fighterName + Api.uiApi.getText("ui.common.allies");
                     }
                     continue;
                  case "enemies":
                     if(Api.fightApi.isSpectator())
                     {
                        fighterName = fighterName + Api.uiApi.getText("ui.common.defenders");
                     }
                     else
                     {
                        fighterName = fighterName + Api.uiApi.getText("ui.common.enemies");
                     }
                     continue;
                  case "all":
                     fighterName = fighterName + Api.uiApi.getText("ui.common.everyone");
                     continue;
                  default:
                     names = "";
                     for each(fighterId in _targets)
                     {
                        names = names + ((names != ""?", ":"") + getFighterCacheName(fighterId));
                     }
                     fighterName = fighterName + names;
                     continue;
               }
            }
         }
