   ...
   public final class TableCellElement extends TableFormattedElement
   {
      ...
      public function compose() : Boolean
      {
         ...
         for(var i:int = 0; i < this.columnSpan; i++)
         {
            ...
            {
               compWidth = compWidth + table.getColumnAt(this.colIndex + i).columnWidth;
            }
         }
      ...
      public function getTotalPaddingWidth() : Number
      {
         ...
         {
            paddingAmount = paddingAmount + table.cellSpacing;
         }
         if(this.textFlow.computedFormat.blockProgression == BlockProgression.RL)
         {
            paddingAmount = paddingAmount + Math.max(getEffectivePaddingTop() + getEffectivePaddingBottom(),getEffectiveBorderTopWidth() + getEffectiveBorderBottomWidth());
         }
         else
         {
            paddingAmount = paddingAmount + Math.max(getEffectivePaddingLeft() + getEffectivePaddingRight(),getEffectiveBorderLeftWidth() + getEffectiveBorderRightWidth());
         }
