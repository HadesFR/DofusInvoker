package Ankama_Connection.ui
{
   ...
   public class CharacterSelection
   {
         ...
         switch(target)
         {
            ...
            case this.tx_bonusXp:
               if(server.gameTypeId == GameServerTypeEnum.SERVER_TYPE_HARDCORE || server.gameTypeId == GameServerTypeEnum.SERVER_TYPE_EPIC)
               {
                  ...
                  {
                     tooltipText = tooltipText + this.uiApi.getText("ui.information.xpHardcoreEpicDeathBonus");
                  }
                  else
                  {
                     tooltipText = tooltipText + this.uiApi.getText("ui.information.xpHardcoreEpicBonus");
                  }
               }
               else
               {
      ...
      public function getNbCharacters(type:int) : int
      {
         ...
         var serversList:Object = this.connecApi.getServers();
         var _nbCharacter:int = 0;
         for(i in serversList)
         {
            currServer = this.dataApi.getServer(serversList[i].id);
            if(currServer.gameTypeId == type)
            {
               _nbCharacter = _nbCharacter + serversList[i].charactersCount;
            }
         }
         return _nbCharacter;
      }
