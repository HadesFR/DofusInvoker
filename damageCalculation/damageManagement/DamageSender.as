package damageCalculation.damageManagement
{
   ...
   public class DamageSender
   {
      ...
      public static function getTotalDamage(param1:FightContext, param2:RunningEffect, param3:Boolean = false) : DamageRange
      {
         ...
         {
            if(int(_loc4_.getCharacteristicValue("movementPoints")) <= 0)
            {
               _loc6_.setToZero();
            }
            else
            {
               _loc7_ = _loc4_.getCharacteristicValue("movementPoints");
      ...
      public static function getTotalShield(param1:RunningEffect, param2:Boolean = false) : DamageRange
      {
         ...
         else if(_loc6_ == 1039)
         {
            ...
            _loc8_ = new Interval(int(_loc3_.getCharacteristicValue("maxLifePoints")),int(_loc3_.getCharacteristicValue("maxLifePoints")));
         ...
         {
            _loc7_ = _loc4_.getDamageInterval();
            _loc8_ = new Interval(_loc3_.level,_loc3_.level).multiplyInterval(_loc7_).multiply(0.01);
            _loc5_.addInterval(_loc8_);
         }
         return _loc5_;
      }
      ...
      public static function getDamageBasedOnCasterLife(param1:RunningEffect, param2:DamageRange) : DamageRange
      {
         ...
         {
            _loc6_ = int(_loc3_.getCharacteristicValue("baseMaxLifePoints")) - int(_loc3_.getCharacteristicValue("maxLifePoints"));
            if(_loc6_ >= 0)
            {
         ...
         else if(ActionIdHelper.isBasedOnCasterLifeMidlife(_loc4_.actionId))
         {
            ...
            _loc7_.min = int(100 * _loc5_.min / int(_loc3_.getCharacteristicValue("maxLifePoints")) - 50);
            _loc7_.max = int(100 * _loc5_.max / int(_loc3_.getCharacteristicValue("maxLifePoints")) - 50);
            _loc7_.abs().minimizeBy(0).maximizeBy(50);
            param2.min = int(param2.min * (int(_loc3_.getCharacteristicValue("baseMaxLifePoints")) * DamageSender.MIDLIFE_DAMAGE_PERCENT[_loc7_.min] / 100));
            param2.max = int(param2.max * (int(_loc3_.getCharacteristicValue("baseMaxLifePoints")) * DamageSender.MIDLIFE_DAMAGE_PERCENT[_loc7_.max] / 100));
         }
         return param2;
      }
      ...
      public static function getBoostableDamage(param1:RunningEffect, param2:DamageRange) : DamageRange
      {
         ...
         param2.multiply(Number(DamageSender.percentDamageBonus(param1)));
         param2.add(int(DamageSender.flatDamageBonus(param1)));
         if(param1.getSpell().isWeapon)
         {
            param2.multiply((100 + int(param1.getCaster().getCharacteristicValue("weaponDamagesBonusPercent"))) * 0.01);
         }
      ...
      public static function flatDamageBonus(param1:RunningEffect) : int
      {
         ...
         var _loc5_:* = (!!ActionIdHelper.isHeal(_loc3_.actionId)?int(_loc2_.getCharacteristicValue("healBonus")):int(_loc2_.getCharacteristicValue("allDamagesBonus"))) + (!!_loc4_.isTrap?int(_loc2_.getCharacteristicValue("trapBonus")):0);
         if(!ActionIdHelper.isHeal(_loc3_.actionId))
         {
            _loc5_ = _loc5_ + (int(_loc2_.getElementFlatDamageBonus(int(ElementEnum.getElementFromActionId(_loc3_.actionId)))) + (!!_loc3_.isCritical && int(param1.getSpell().getEffects().indexOf(_loc3_)) == -1?int(_loc2_.getCharacteristicValue("criticalDamageBonus")):0));
         }
         return _loc5_;
      }
      ...
      public static function percentDamageBonus(param1:RunningEffect) : Number
      {
         var _loc2_:HaxeFighter = param1.getCaster();
         var _loc3_:HaxeSpellEffect = param1.getSpellEffect();
         var _loc4_:HaxeSpell = param1.getSpell();
         ...
         var _loc6_:* = (!!ActionIdHelper.isHeal(_loc3_.actionId)?0:int(_loc2_.getCharacteristicValue("damagesBonusPercent"))) + (!!_loc4_.isTrap?int(_loc2_.getCharacteristicValue("trapBonusPercent")):0) + (!!_loc4_.isGlyph?int(_loc2_.getCharacteristicValue("glyphPower")):0) + (!!_loc4_.isRune?int(_loc2_.getCharacteristicValue("runePower")):0) + (!!_loc4_.isWeapon?int(_loc2_.getCharacteristicValue("weaponPower")):int(_loc2_.getCharacteristicValue("spellPercentDamages")) + int(_loc2_.getSpellDamageModification(param1.getSpell().id))) + int(_loc2_.getElementMainStat(_loc5_));
         if(_loc6_ < 0)
         {
            _loc6_ = 0;
         }
         return (100 + _loc6_) * 0.01;
      }
      
      public static function bombComboBonus(param1:RunningEffect) : Number
      {
         return Number(1 + int(param1.getCaster().getCharacteristicValue("bombCombo")) / 100);
      }
   }
}
