package Ankama_Cartography.ui
{
   ...
   public class CartographyUi extends CartographyBase
   {
      ...
      override public function main(params:Object = null) : void
      {
         ...
         var bannerMap:* = uiApi.getUi("bannerMap");
         if(bannerMap)
         {
      ...
      protected function updateAllFilters() : void
      {
         ...
         var select:* = filters.some(isSelected);
      ...
      public function onSelectItem(target:Object, method:uint, isNew:Boolean) : void
      {
         ...
         else if(target == this.gdSearchAll && (method == SelectMethodEnum.MANUAL || method == SelectMethodEnum.AUTO))
         {
            ...
            this.gdSearchAll.updateItem(target.selectedIndex);
         }
      ...
      public function onItemRollOver(target:Object, item:Object) : void
      {
         ...
         if(item.data && item.data.data && item.data.type == "subarea")
         {
            ...
            if(currentMapElement != __lastHighlightElement)
            {
               ...
               if(item.data.hasAnomaly)
               {
                  ...
                  anomaly = mapViewer.getMapElement(__lastHighlightAnomaly);
                  if(anomaly)
                  {
         ...
         else if(item.data && item.data.data && item.data.type == "dungeon")
         {
            ...
            {
               __lastHighlightElement = currentMapElement;
               mapElement = mapViewer.getMapElement(__lastHighlightElement);
               if(mapElement)
               {
      ...
      override public function onRollOver(target:Object) : void
      {
         ...
         if(target.name.indexOf("tx_itemType") != -1 && target.uri)
         {
            if(target.uri.fileName == "success_cat_5.png")
            {
               tooltipText = uiApi.getText("ui.common.collectableResource");
            }
            else if(target.uri.fileName == "success_cat_6.png")
            {
      ...
      override public function onMapRollOver(pTarget:Object, pX:int, pY:int, pSearchSubArea:SubArea = null) : void
      {
         ...
         if(searchSubArea && !hintSubArea)
         {
            ...
            searchSubAreaText = searchSubAreaText + searchInfo;
         }
         if(hintSubArea)
         {
            searchSubAreaText = searchSubAreaText + this.getSubAreaInfoStr(hintSubArea,hintSubArea.name);
         }
         if(searchSubArea || hintSubArea)
         {
      ...
      private function getSubAreaInfoStr(pSubArea:SubArea, pSubAreaName:String) : String
      {
         ...
         if(this._searchSelectedItem.resourceSubAreaIds)
         {
            ...
            searchInfo = searchInfo + (uiApi.getText("ui.common.quantity") + uiApi.getText("ui.common.colon") + itemQty);
         }
      ...
      private function createPrismsSearchGroup(pPrisms:Object) : void
      {
      ...
      private function updatePrismsSearchGroup(pPrismsIds:Object) : void
      {
      ...
      {
         var dungeons:Object = dataApi.queryString(Dungeon,"name",pDungeonName);
      ...
      {
         var hints:Object = dataApi.queryString(Hint,"name",pHintName);
         if(hints.length > 0)
         {
            return hints[0];
         }
