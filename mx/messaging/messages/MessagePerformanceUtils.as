package mx.messaging.messages
{
   public class MessagePerformanceUtils
   {
      ...
      public function prettyPrint() : String
      {
         ...
         {
            alertString = alertString + ("Original message size(B): " + this.messageSize + "\n");
         }
         if(this.responseMessageSize != 0)
         {
            alertString = alertString + ("Response message size(B): " + this.responseMessageSize + "\n");
         }
         if(this.totalTime != 0)
         {
            alertString = alertString + ("Total time (s): " + this.totalTime / 1000 + "\n");
         }
         if(this.networkRTT != 0)
         {
            alertString = alertString + ("Network Roundtrip time (s): " + this.networkRTT / 1000 + "\n");
         }
         if(this.serverProcessingTime != 0)
         {
            alertString = alertString + ("Server processing time (s): " + this.serverProcessingTime / 1000 + "\n");
         }
         if(this.serverAdapterTime != 0)
         {
            alertString = alertString + ("Server adapter time (s): " + this.serverAdapterTime / 1000 + "\n");
         }
         if(this.serverNonAdapterTime != 0)
         {
            alertString = alertString + ("Server non-adapter time (s): " + this.serverNonAdapterTime / 1000 + "\n");
         }
         if(this.serverAdapterExternalTime != 0)
         {
            alertString = alertString + ("Server adapter external time (s): " + this.serverAdapterExternalTime / 1000 + "\n");
         }
         if(this.pushedMessageFlag)
         {
            alertString = alertString + "PUSHED MESSAGE INFORMATION:\n";
            if(this.totalPushTime != 0)
            {
               alertString = alertString + ("Total push time (s): " + this.totalPushTime / 1000 + "\n");
            }
            if(this.pushOneWayTime != 0)
            {
               alertString = alertString + ("Push one way time (s): " + this.pushOneWayTime / 1000 + "\n");
            }
            if(this.originatingMessageSize != 0)
            {
               alertString = alertString + ("Originating Message size (B): " + this.originatingMessageSize + "\n");
            }
            if(this.serverPollDelay != 0)
            {
               alertString = alertString + ("Server poll delay (s): " + this.serverPollDelay / 1000 + "\n");
            }
         }
         return alertString;
      }
