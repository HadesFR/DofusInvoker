   ...
   public class StringFormatter
   {
      ...
      public function formatValue(value:Object) : String
      {
         ...
         for(var i:int = 1; i < n; i++)
         {
            ...
            result = result + (this.reqFormat.substring(lastTokenInfo.end,curTokenInfo.begin) + this.extractToken(value,curTokenInfo));
            lastTokenInfo = curTokenInfo;
         }
         if(lastTokenInfo.end > 0 && lastTokenInfo.end != this.reqFormat.length)
         {
            result = result + this.reqFormat.substring(lastTokenInfo.end);
         }
         return result;
      }
