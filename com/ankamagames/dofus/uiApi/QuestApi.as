   ...
   public class QuestApi implements IApi
   {
      ...
      public function getAchievementRewardsText(achievementRewards:AchievementRewardsWrapper) : String
      {
         ...
         for each(reward in achievementRewards.rewardsList)
         {
            ...
            {
               result = result + ("\r" + I18n.getUiText("ui.common.item",[Item.getItemById(id).name]));
            }
            for each(id in reward.emotesReward)
            {
               result = result + ("\r" + I18n.getUiText("ui.common.emote",[Emoticon.getEmoticonById(id).name]));
            }
            for each(id in reward.ornamentsReward)
            {
               result = result + ("\r" + I18n.getUiText("ui.common.ornament",[Ornament.getOrnamentById(id).name]));
            }
            for each(id in reward.spellsReward)
            {
               result = result + ("\r" + I18n.getUiText("ui.common.spell",[Spell.getSpellById(id).name]));
            }
            for each(id in reward.titlesReward)
            {
               result = result + ("\r" + I18n.getUiText("ui.common.title",[Title.getTitleById(id).name]));
            }
         }
         if(result.length > 0)
         {
