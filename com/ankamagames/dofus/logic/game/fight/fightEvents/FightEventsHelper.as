package com.ankamagames.dofus.logic.game.fight.fightEvents
{
   ...
   public class FightEventsHelper
   {
      ...
      public static function removeFightEvent(name:String, targetId:Number, buff:BasicBuff) : void
      {
         var j:uint = 0;
         if(_events === null)
         {
         ...
         var fightEvents:Vector.<FightEvent> = null;
         var fightEvent:FightEvent = null;
         var eventsLength:uint = _events.length;
         var fightEventsLength:uint = 0;
         for(var i:uint = 0; i < eventsLength; i++)
         {
            fightEvents = _events[i];
            if(fightEvents !== null)
            {
               fightEventsLength = fightEvents.length;
               for(j = 0; j < fightEventsLength; j++)
               {
                  fightEvent = fightEvents[j];
                  if(fightEvent !== null)
                  {
                     if(fightEvent.name === name && fightEvent.targetId === targetId && fightEvent.buff.id === buff.id)
                     {
                        fightEvents.removeAt(j);
