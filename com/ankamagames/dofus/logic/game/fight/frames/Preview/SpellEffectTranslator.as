package com.ankamagames.dofus.logic.game.fight.frames.Preview
{
   ...
   public class SpellEffectTranslator extends HaxeSpellEffect
   {
      ...
      {
         if(effect.effectUid != 0 && _cache[effect.effectUid])
         {
            return _cache[effect.effectUid];
         }
      ...
      public static function fromWeapon(effect:EffectInstance, weapon:WeaponWrapper, isCritical:Boolean) : SpellEffectTranslator
      {
         ...
         if(weapon != null)
         {
            ...
            {
               zone = zone + ((effect.zoneMinSize || 0).toString() + ",");
            }
            zone = zone + ((effect.zoneEfficiencyPercent || 25).toString() + "," + (effect.zoneMaxEfficiency || 1).toString());
            if(isCritical && ActionIdHelper.isDamage(effect.category))
            {
               param0 = param0 + weapon.criticalHitBonus;
               param1 = param1 + weapon.criticalHitBonus;
            }
         }
