package com.ankamagames.dofus.logic.common.managers
{
   ...
   public class AbstractItemFilterManager
   {
      ...
      private const TEMPORIS_SCROLL_TYPE_ID:uint = 223;
      ...
      protected var _currentFilteredItemIds:Vector.<uint> = null;
      ...
      {
         var id:uint = 0;
         var effectsId:Array = [DataEnum.ITEM_TYPE_COLLAR,DataEnum.ITEM_TYPE_RING,DataEnum.ITEM_TYPE_BELT,DataEnum.ITEM_TYPE_SHOES,DataEnum.ITEM_TYPE_SHIELD,DataEnum.ITEM_TYPE_HAT,DataEnum.ITEM_TYPE_CLOAK,DataEnum.ITEM_TYPE_BACKPACK,DataEnum.ITEM_TYPE_PET,DataEnum.ITEM_TYPE_PETSMOUNT];
         this._targetEquipment = [];
         for each(id in effectsId)
         {
      ...
      public function addSubCategory(superTypeId:Array, filterCatText:String = "", specificTypes:Vector.<uint> = null) : Array
      {
         ...
         if(Vector.<uint>(subTypeIds).length > 0)
         {
            ...
            {
               if(!(PlayerManager.getInstance().server.gameTypeId != GameServerTypeEnum.SERVER_TYPE_TEMPORIS && subTypeId == this.TEMPORIS_SCROLL_TYPE_ID))
               {
      ...
      public function filterItems(searchCriteria:String) : Array
      {
         ...
         for each(obtainingId in this._obtainingMethod)
         {
            ...
            {
               this._currentUi.uiClass.sysApi.getPlayerManager().serverSeason = this._currentUi.uiClass.sysApi.getCurrentServer().gameTypeId == GameServerTypeEnum.SERVER_TYPE_TEMPORIS?this._dataApi.getCurrentSeason():null;
      ...
      public function filterRunesFromItemEffects(characsIdList:Vector.<uint> = null) : Array
      {
         var characId:uint = 0;
         ...
         result = this._dataApi.queryEquals(Item,"typeId",DataEnum.ITEM_TYPE_SMITHMAGIC_RUNE);
         ...
         result = this._dataApi.queryIntersection(result,resultItemWithEffect);
         ids = [];
