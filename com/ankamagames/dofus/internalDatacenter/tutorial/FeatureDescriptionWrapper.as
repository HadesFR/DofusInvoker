package com.ankamagames.dofus.internalDatacenter.tutorial
{
   ...
   public class FeatureDescriptionWrapper implements IDataCenter
   {
      ...
      private function createDescription() : void
      {
         ...
         var node:XML = null;
         var xmlText:* = "<desc>" + this.formatText(this.description) + "</desc>";
         ...
         if(this.parentId != 0)
         {
            ...
            this.totalHeight = this.totalHeight + subTitleLabel.contentHeight;
         }
         else
         {
         ...
         for each(node in formatedDescription.children())
         {
            ...
            {
               subTitleLabel.x = subTitleLabel.x + node.attribute("x");
            }
         }
      ...
      public function highlight(block:TextField, search:String, parentLabel:Label) : void
      {
         ...
         {
            this.searchResult.push(new SearchResult#8554(positions[i].posStart,positions[i].posEnd,block,parentLabel));
      ...
      private function decodeShortCut(t:String) : String
      {
         ...
         while(true)
         {
            ...
            if(shortcut)
            {
               ...
               if(shortcutString != "")
               {
                  currentText = currentText + ("(" + shortcutString + ")");
               }
...
class SearchResult#8554
{
   ...
   function SearchResult#8554(pStart:int, pEnd:int, pTextField:TextField, pParentLabel:Label)
   {
      super();
