   ...
   public class AuctionHouseSell extends BasicItemCard
   {
      ...
      private function parseObjectsInfos(objectsInfos:Object) : void
      {
         ...
         for each(objectInfo in objectsInfos)
         {
            ...
            this._totalObjectPrice = this._totalObjectPrice + object.price;
      ...
      override public function onRelease(target:Object) : void
      {
         ...
         switch(target)
         {
            ...
            case btn_remove:
               if(!this._mode)
               {
                  this._currentObjectToRemove = new Object();
      ...
      public function onSelectItem(target:Object, selectMethod:uint, isNewSelection:Boolean) : void
      {
         switch(target)
         {
            ...
            case this.gd_itemList:
               if(selectMethod != GridItemSelectMethodEnum.AUTO && target.selectedItem && !uiApi.keyIsDown(Keyboard.SHIFT))
               {
                  this.onSelectItemFromStockItemList(target.selectedItem);
               }
               break;
            case this.cb_stack:
               this._currentStackFilter = int(this.cb_stack.value);
      ...
      private function onConfirmSellObject() : void
      {
         ...
         var txt:Object = uiApi.textTooltipInfo(uiApi.getText("ui.bidhouse.successSell"));
         uiApi.showTooltip(txt,btn_valid,true,"standard",7,1,3,null,null,null,"TextInfo");
      }
      ...
      public function onConfirmModifyMultipleItems() : void
      {
         ...
         for each(item in this._allItemsToRemove)
         {
            if(item.price != this._price)
            {
               ...
               {
                  TradeCenter.SALES_PRICES[item.itemWrapper.objectGID] = new Array();
               }
